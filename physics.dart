...
SimResult runSimulation(BowlerInputs inp, PatternData pat, BallSpecs ball, List<List<double>> oilMatrix) {
  ...
  while (ft < LANE_FT.toDouble() && vx > 0.3) {
    ...
    // Hersey-number friction model
    final double muMax = ball.muDry * (1.0 + 0.12 * flare);
    final double muMin = MU_OIL_MIN;

    // Simple effective Hersey number
    final double H = oilLocal * slipMag;

    // Transition function
    const double kH = 8.0; //constant adjust to tweak to right value

    final double muEff = muMin + (muMax - muMin) * exp(-kH * H);

    // Keep a small static-vs-kinetic split
    final double muK = muEff * 0.90;
    final double muS = muEff;
    ...
    if (inGutter) {
      ...
      // if (omegaF > 200.0) omegaF = 200.0;
      // if (omegaH.abs() > 200.0) omegaH = 200.0 * omegaH.sign;
