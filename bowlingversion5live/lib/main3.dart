...
//deafault startup for 3d real
  ...
  // Static standing 10-pin rack at 60 ft (USBC triangle, 12" spacing).
  // Phase 2 item 7 will replace with live physics sim state.
  static const double _headPinFt = 60.0;
  static const double _laneWidthIn = 41.5;
  // Center pin on middle board (board 19 of 0..38). 12" spacing in board units:
  // 12 in / (41.5 in / 38 boards) ≈ 10.99 boards per pin-row offset; halve for +/- => 5.49
  static final List<_PinRenderState> _defaultPinStates = () {
    const double boardsPerInch = 38.0 / 41.5;
    const double spB = 12.0 * boardsPerInch;       // board delta for 12"
    const double spF = 12.0 / 12.0 * 0.866;        // ft delta (triangle height = 12*sqrt(3)/2 in = 10.39 in ≈ 0.866 ft)
    const double cb = 19.0;                         // center board
    // Rows: 1, 2, 3, 4 pins; back row is farthest from shooter (higher ft)
    final rows = <List<double>>[
      [0],                // row 0 (head pin)
      [-0.5, 0.5],        // row 1
      [-1.0, 0.0, 1.0],   // row 2
      [-1.5, -0.5, 0.5, 1.5], // row 3
    ];
...
// Reuses projection math mirrored from _3DOilPainter.
// ============================================================
class _3DLanePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double pxPerInchY = laneD / (laneLengthFt * 12.0);
    ...
    final Color laneColor = const Color(0xFFE9C98A);
    final Color gutterColor = const Color(0xFF2B2B2F);
    ...
    // ---- 2. OIL OVERLAY (0 -> oilDistance only) ----
    ...
    // ---- 5. BALL PATH (trail only — ball drawn after depth sort) ----
    // Ball + path drawn after the face sort so they always render on top.

    // ---- 6. PINS ----
    final double boardsPerMeter = (1.0 / LANE_WIDTH_IN) * 39.37 * (BOARDS - 1);
    final double ftPerMeter = 39.37 / 12.0;
    // world-z per meter of pin height (match X scale visually)
    final double zPerMeterNorm = (pxPerInchX * 39.37) / heightScale;

    for (final ps in pinStates) {
      if (!ps.visible) continue;
      _drawPin3D(faces, ps, world, proj,
    ...
    // ---- Ball path + ball (drawn last, always on top) ----
    if (path.isNotEmpty) {
  ...
  void _drawPin3D(
    ...
    for (int r = 0; r <= rings; r++) {
      ...
      // Tilt: pin falls forward in +ft direction by ps.tiltRad
      final double cosT = cos(ps.tiltRad);
      final double sinT = sin(ps.tiltRad);
      ...
      for (int k = 0; k < ribs; k++) {
        ...
        // Untilted local (meters): (dxM, dzM horizontal, localY vertical)
        // Apply tilt around X: y' = localY*cos - dzM*sin, z' = localY*sin + dzM*cos (approx)
        final double localFtM = dzM * cosT - localY * sinT * 0; // keep horizontal offset
        final double localYTiltM = localY * cosT + dzM * sinT * 0;
        final double tiltFtAddM = localY * sinT;

        final double boardOff = dxM * boardsPerMeter;
        final double ftOff = (localFtM + tiltFtAddM) * ftPerMeter;
        final double zW = localYTiltM * zPerMeterNorm * heightScale;
