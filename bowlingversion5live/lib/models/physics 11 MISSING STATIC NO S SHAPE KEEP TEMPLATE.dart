...
SimResult runSimulationV2(BowlerInputs inp, PatternData pat, BallSpecs ball, OilMatrix oil) {
  ...
  while (ft < LANE_FT.toDouble() && vx > 0.3) {
    ...
    // Cheap hydrodynamic-contact proxy: more oil spreads load over a larger
    // effective patch, reducing pressure and increasing the local Hersey number.
    final double film = hLocal.clamp(0.0, 1.0);
    final double contactAreaEff = CONTACT_AREA_M2 * (1.0 + OIL_AREA_GAIN * film);
    final double pressure = fn / contactAreaEff;
    ...
    final double muRaw = MU_OIL_MIN +
        (STRIBECK_A * surface.aScale) / (hHat + STRIBECK_B) +
        STRIBECK_C * (1.0 - exp(-STRIBECK_D * hHat));
    final double muEff = muRaw.clamp(MU_ROLL_RES, muCap);
    ...
    final double depletionRate = OIL_DEPLETION_K * muEff * fn * slipMag / (OIL_DENSITY_PROXY * contactAreaEff);
