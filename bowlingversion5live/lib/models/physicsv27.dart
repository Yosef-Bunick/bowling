...
const double MU_K_DRY_SCALE = 0.5;//change for ealier or later hook 
...
OilMatrix buildOilMatrix(
  ...
  double applyForwardRows(List<LoadRow> rows, List<List<double>> target) {
    ...
    for (final row in rows) {
      ...
      final double baseAmount = row.toil / (numBoards * loadZoneLen * 50.0);
      ...
      for (int b = bStart; b <= bEnd; b++) {
        for (int col = 0; col < OIL_COLS; col++) {
          ...
          if (x < x0) {
            ...
            final double t = ((x - x0) / loadZoneLen).clamp(0.0, 1.0);

            const double edge = 0.18;
            double shape;

            if (t < edge) {
              final double u = t / edge;
              shape = u * u * (3.0 - 2.0 * u);
            } else if (t > 1.0 - edge) {
              final double u = (1.0 - t) / edge;
              shape = u * u * (3.0 - 2.0 * u);
            } else {
              shape = 1.0;
            }

            contribution = (baseAmount + gamma * saturation) * shape;
          } else {
            final double d = x - x1;
            if (d <= buffFt) {
              contribution = endSaturation * alphaF * pow(rF, d).toDouble();
            }
          }
      ...
      for (int b = bStart; b <= bEnd; b++) {
        for (int col = 0; col < OIL_COLS; col++) {
          ...
          if (x > xDrop) {
            ...
            final double t = ((x - xStop) / loadZoneLen).clamp(0.0, 1.0);
            ...
            if (d <= buffFt && d >= 0) {
              contribution = endSaturation * alphaR * pow(rR, d).toDouble();
            }
          }
          target[b][col] += contribution;
        }
  ...
  for (int b = 0; b < BOARDS; b++) {
    for (int col = 0; col < OIL_COLS; col++) {
      fwdRaw[b][col] = fwdRaw[b][col].clamp(0.0, 1.0);
      revRaw[b][col] = revRaw[b][col].clamp(0.0, 1.0);
    }
  }
