...
SimResult runSimulation(BowlerInputs inp, PatternData pat, BallSpecs ball, List<List<double>> oilMatrix) {
  ...
  while (ft < LANE_FT.toDouble() && vx > 0.3) {
    ...
    if (inGutter) {
      ...
      final double rollSurface = BALL_R_M * omegaF;
      final double slipRoll = speedMag - rollSurface;
      final double slipSign = slipRoll.abs() > 1e-8 ? slipRoll.sign : 0.0;

      // Keep longitudinal coupling alive in roll so speed can spin revs up
      // and revs can feed speed forward, while the heading stays locked.
      final double FxRoll = -muX * Fn * slipSign;
      final double decel = MU_ROLL_RES * Fn / massKg;
      final double axRoll = FxRoll / massKg;
      final double alphaF = -(BALL_R_M * FxRoll) / Ieff;

      final double nextSpeed = max(0.3, speedMag + axRoll * dt - decel * dt);
      omegaF += alphaF * dt;
      if (omegaF < 0.0) omegaF = 0.0;
