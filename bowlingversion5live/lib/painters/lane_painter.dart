  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (oilMatrix.isNotEmpty) {
      ...
      for (int col = 0; col < cols; col++) {
        final double ftPos = col * res;
        if (ftPos >= LANE_FT) continue;
        for (int b = 0; b < BOARDS && b < oilMatrix.length; b++) {
          if (col >= oilMatrix[b].length) continue;
          final v = oilMatrix[b][col];
          if (v > 0.02) {
            canvas.drawRect(
              Rect.fromLTWH(b * bw, (LANE_FT - ftPos - res) * fh, bw, res * fh),
...
class OilMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Detect resolution from matrix column count
    final int cols = oilMatrix.first.length;
    final double res = cols > 65 ? 0.25 : 1.0;  // 260 cols = 0.25 ft, 65 cols = 1 ft
    final int distCols = (dist / res).round();  // columns to render
    final double fh = size.height / dist;  // pixels per foot (for display)

    // Oil cells — pins at top, foul line at bottom
    for (int col = 0; col < distCols && col < cols; col++) {
      final double ftPos = col * res;  // position in feet
      for (int b = 0; b < BOARDS && b < oilMatrix.length; b++) {
        final v = oilMatrix[b][col];
        if (v > 0.01) {
          canvas.drawRect(
            Rect.fromLTWH(
              b * bw,
              (dist - ftPos - res) * fh,  // flip Y, account for cell height
              bw + 0.5,
              res * fh + 0.5,  // cell height in pixels
            ),
